                ...
                <div class="form-group">
                    <form id="frm-modal" action="#" enctype="multipart/form-data">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="agent_name" class="form-label">Nama Tandon</label>
                                    <input hidden id="idm" name="idm">
                                    <input class="form-control" type="text" name="nama_tandon" id="nama_tandon" required>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#table').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('master/ajax_data_tandon')?>",
                "type": "POST"
            },
    ...
    function edit(id){
        ...
        $.ajax({
            url : "<?php echo site_url('master/editTandon/')?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {		
                $('#md-form').modal('show'); // show bootstrap modal when complete loaded
                $('.modal-title').text('Edit Data Tandon'); // Set title to Bootstrap modal title

                $('#idm').val(data.idm);
                $('#nama_tandon').val(data.nama_tandon);
    ...
    function save(){    
        ...
        if(save_method == 'add') {
            ...
            url = "<?php echo site_url('master/input_data_tandon');?>"; 
        } else {
            $('#btnSave').text('Updating...'); //change button text
            $('#btnSave').attr('disabled',true); //set button disable 
            url = "<?php echo site_url('master/edit_tandon');?>"; 
        }
    ...
    function add(){
        ...
        $('#md-form').modal('show'); // show bootstrap modal when complete loaded
        $('.modal-title').text('Tambah Data Tandon'); // Set title to Bootstrap modal title
    }
    ...
    function delete_data(id) {
        if(confirm('Apakah Anda Yakin Untuk Menghapus Data Ini?'))
        {
            ...
            $.ajax({
                url : "<?php echo site_url('master/delete_data_tandon')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
